<?php
/*
Plugin Name: Simple OrgChart
Description: A simple OrgChart plugin using Balkan OrgChart.js with data update via fetch.
Version: 1.0
Author: Your Name
*/

/**
 * Enqueue scripts and styles for the Simple OrgChart plugin.
 *
 ...
 * our custom CSS file.
 */
function simple_orgchart_enqueue_scripts() {
    ...
    wp_enqueue_script( 'simple-orgchart-script', plugin_dir_url( __FILE__ ) . 'simple-orgchart-script.js', array('balkan-orgchart'), '1.0', true );

    // Enqueue our custom CSS file.
    wp_enqueue_style( 'simple-orgchart-style', plugin_dir_url( __FILE__ ) . 'simple-orgchart-style.css', array(), '1.0', 'all' );


    // Localize script: Pass PHP variables to our JavaScript.
    // 'orgchart_ajax' will be a global JavaScript object.
    wp_localize_script( 'simple-orgchart-script', 'orgchart_ajax', array(
        ...
        'initial_data' => get_option( 'simple_orgchart_data', '[]' )
    ) );
}
add_action( 'wp_enqueue_scripts', 'simple_orgchart_enqueue_scripts' );

/**
 * Shortcode to display the OrgChart.
 *
 * This function creates the `[simple_orgchart]` shortcode. When the shortcode is
...
function simple_orgchart_update_data() {
    ...
    $current_data = json_decode( get_option( 'simple_orgchart_data', '[]' ), true );
    ...
    switch ( $action_type ) {
        case 'add':
            ...
            if ( $new_node ) {
                ...
                $response['success'] = update_option( 'simple_orgchart_data', json_encode( $current_data ) );
...
add_action( 'wp_ajax_simple_orgchart_update', 'simple_orgchart_update_data' );
...
add_action( 'wp_ajax_nopriv_simple_orgchart_update', 'simple_orgchart_update_data' );
 ...
 */
function simple_orgchart_activate() {
    // Check if the option 'simple_orgchart_data' does not exist (false return).
    if ( get_option( 'simple_orgchart_data' ) === false ) {
        ...
        update_option( 'simple_orgchart_data', json_encode( $default_data ) );
    }
}
// Register the activation hook.
register_activation_hook( __FILE__, 'simple_orgchart_activate' );
 ...
 */
function simple_orgchart_deactivate() {
    // Delete the option that stores the OrgChart data.
    delete_option( 'simple_orgchart_data' );
}
// Register the deactivation hook.
register_deactivation_hook( __FILE__, 'simple_orgchart_deactivate' );
